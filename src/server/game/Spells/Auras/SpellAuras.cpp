        ...
        switch (GetSpellInfo()->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_MAGE:
            {
                ...
                switch (GetId())
                {
                    ...
                    {
                        UnitList targets;
                        MistCore::AnyUnitHavingBuffInObjectRangeCheck u_check(caster, caster, 300.0f, 44457, false);
                        MistCore::UnitListSearcher<MistCore::AnyUnitHavingBuffInObjectRangeCheck> searcher(caster, targets, u_check);
                        ...
                        if (targets.size() >= 4)
                        {
                            ...
                            if (auras.size() >= 4)
                            {
                                auras.sort(MistCore::DurationOrderPred(false));
                                auras.pop_front();
                                auras.pop_front();
                                auras.pop_front();
...
bool Aura::CallScriptCheckAreaTargetHandlers(Unit* target)
{
    ...
    if (scriptExecuteTime > 10)
        sLog->outWarn(LOG_FILTER_SPELLS_AURAS, "AuraScript [%u] take more than 10 ms to execute (%u ms)", GetId(), scriptExecuteTime);
        ...
        else
        {
            ...
            if (!GetUnitOwner()->HasUnitState(UNIT_STATE_ISOLATED))
            {
                switch (GetSpellInfo()->Effects[effIndex].Effect)
                {
                    ...
                    {
                        targetList.push_back(GetUnitOwner());
                        MistCore::AnyGroupedUnitInObjectRangeCheck u_check(GetUnitOwner(), GetUnitOwner(), radius, GetSpellInfo()->Effects[effIndex].Effect == SPELL_EFFECT_APPLY_AREA_AURA_RAID);
                        MistCore::UnitListSearcher<MistCore::AnyGroupedUnitInObjectRangeCheck> searcher(GetUnitOwner(), targetList, u_check);
                    ...
                    {
                        targetList.push_back(GetUnitOwner());
                        MistCore::AnyFriendlyUnitInObjectRangeCheck u_check(GetUnitOwner(), GetUnitOwner(), radius);
                        MistCore::UnitListSearcher<MistCore::AnyFriendlyUnitInObjectRangeCheck> searcher(GetUnitOwner(), targetList, u_check);
                        GetUnitOwner()->VisitNearbyObject(radius, searcher);
                        break;
                    }
                    case SPELL_EFFECT_APPLY_AREA_AURA_ENEMY:
                    {
                        MistCore::AnyAoETargetUnitInObjectRangeCheck u_check(GetUnitOwner(), GetUnitOwner(), radius); // No GetCharmer in searcher
                        MistCore::UnitListSearcher<MistCore::AnyAoETargetUnitInObjectRangeCheck> searcher(GetUnitOwner(), targetList, u_check);
                        GetUnitOwner()->VisitNearbyObject(radius, searcher);
                        break;
                    }
...
void DynObjAura::FillTargetMap(std::map<Unit*, uint32> & targets, Unit* /*caster*/)
{
    ...
    for (uint8 effIndex = 0; effIndex < MAX_SPELL_EFFECTS; ++effIndex)
    {
        ...
        {
            MistCore::AnyFriendlyUnitInObjectRangeCheck u_check(GetDynobjOwner(), dynObjOwnerCaster, radius);
            MistCore::UnitListSearcher<MistCore::AnyFriendlyUnitInObjectRangeCheck> searcher(GetDynobjOwner(), targetList, u_check);
            GetDynobjOwner()->VisitNearbyObject(radius, searcher);
        }
        else if (GetSpellInfo()->Effects[effIndex].Effect != SPELL_EFFECT_CREATE_AREATRIGGER)
        {
            MistCore::AnyAoETargetUnitInObjectRangeCheck u_check(GetDynobjOwner(), dynObjOwnerCaster, radius);
            MistCore::UnitListSearcher<MistCore::AnyAoETargetUnitInObjectRangeCheck> searcher(GetDynobjOwner(), targetList, u_check);
            GetDynobjOwner()->VisitNearbyObject(radius, searcher);
        }
