        ...
        case GO_READY:
        {
            ...
            if (isSpawned())
            {
                ...
                if (goInfo->type == GAMEOBJECT_TYPE_TRAP)
                {
                    ...
                    {
                        MistCore::AnyUnfriendlyNoTotemUnitInObjectRangeCheck checker(this, owner, radius);
                        MistCore::UnitSearcher<MistCore::AnyUnfriendlyNoTotemUnitInObjectRangeCheck> searcher(this, ok, checker);
                    ...
                    else                                        // environmental trap
                    {
                        ...
                        // affect only players
                        Player* player = NULL;
                        MistCore::AnyPlayerInObjectRangeCheck checker(this, radius);
                        MistCore::PlayerSearcher<MistCore::AnyPlayerInObjectRangeCheck> searcher(this, player, checker);
...
void GameObject::TriggeringLinkedGameObject(uint32 trapEntry, Unit* target)
{
    ...
    GameObject* trapGO = NULL;
    {
        ...
        MistCore::NearestGameObjectEntryInObjectRangeCheck go_check(*target, trapEntry, range);
        MistCore::GameObjectLastSearcher<MistCore::NearestGameObjectEntryInObjectRangeCheck> checker(this, trapGO, go_check);

        TypeContainerVisitor<MistCore::GameObjectLastSearcher<MistCore::NearestGameObjectEntryInObjectRangeCheck>, GridTypeMapContainer > object_checker(checker);
...
GameObject* GameObject::LookupFishingHoleAround(float range)
{
    ...
    CellCoord p(MistCore::ComputeCellCoord(GetPositionX(), GetPositionY()));
    Cell cell(p);
    MistCore::NearestGameObjectFishingHole u_check(*this, range);
    MistCore::GameObjectSearcher<MistCore::NearestGameObjectFishingHole> checker(this, ok, u_check);

    TypeContainerVisitor<MistCore::GameObjectSearcher<MistCore::NearestGameObjectFishingHole>, GridTypeMapContainer > grid_object_checker(checker);
            ...
            if (GetUniqueUseCount() == info->summoningRitual.reqParticipants)
            {
                ...
                if (info->summoningRitual.casterTargetSpell && info->summoningRitual.casterTargetSpell != 1) // No idea why this field is a bool in some cases
                    for (uint32 i = 0; i < info->summoningRitual.casterTargetSpellTargets; i++)
                        // m_unique_users can contain only player GUIDs
                        if (Player* target = ObjectAccessor::GetPlayer(*this, MistCore::Containers::SelectRandomContainerElement(m_unique_users)))
