...
GameObject* ChatHandler::GetNearbyGameObject()
{
    ...
    GameObject* obj = NULL;
    MistCore::NearestGameObjectCheck check(*pl);
    MistCore::GameObjectLastSearcher<MistCore::NearestGameObjectCheck> searcher(pl, obj, check);
...
GameObject* ChatHandler::GetObjectGlobalyWithGuidOrNearWithDbGuid(uint32 lowguid, uint32 entry)
{
    ...
    if (!obj && sObjectMgr->GetGOData(lowguid))                   // guid is DB guid of object
    {
        ...
        CellCoord p(MistCore::ComputeCellCoord(pl->GetPositionX(), pl->GetPositionY()));
        Cell cell(p);

        MistCore::GameObjectWithDbGUIDCheck go_check(*pl, lowguid);
        MistCore::GameObjectSearcher<MistCore::GameObjectWithDbGUIDCheck> checker(pl, obj, go_check);

        TypeContainerVisitor<MistCore::GameObjectSearcher<MistCore::GameObjectWithDbGUIDCheck>, GridTypeMapContainer > object_checker(checker);
