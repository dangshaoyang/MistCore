...
void Map::SwitchGridContainers(Creature* obj, bool on)
{
    ...
    CellCoord p = MistCore::ComputeCellCoord(obj->GetPositionX(), obj->GetPositionY());
    if (!p.IsCoordValid())
    {
...
{
    CellCoord cellCoord = MistCore::ComputeCellCoord(player->GetPositionX(), player->GetPositionY());
...
bool Map::AddToMap(T *obj)
{
    ...
    CellCoord cellCoord = MistCore::ComputeCellCoord(obj->GetPositionX(), obj->GetPositionY());
...
void Map::VisitNearbyCellsOf(WorldObject* obj, TypeContainerVisitor<MistCore::ObjectUpdater, GridTypeMapContainer> &gridVisitor, TypeContainerVisitor<MistCore::ObjectUpdater, WorldTypeMapContainer> &worldVisitor)
{
...
void Map::Update(const uint32 t_diff)
{
    ...
    MistCore::ObjectUpdater updater(t_diff);
    // for creature
    TypeContainerVisitor<MistCore::ObjectUpdater, GridTypeMapContainer  > grid_object_update(updater);
    // for pets
    TypeContainerVisitor<MistCore::ObjectUpdater, WorldTypeMapContainer > world_object_update(updater);
...
{
    cell.SetNoCreate();
    MistCore::VisibleChangesNotifier notifier(*obj);
    TypeContainerVisitor<MistCore::VisibleChangesNotifier, WorldTypeMapContainer > player_notifier(notifier);
...
{
    MistCore::VisibleNotifier notifier(*player);

    cell.SetNoCreate();
    TypeContainerVisitor<MistCore::VisibleNotifier, WorldTypeMapContainer > world_notifier(notifier);
    TypeContainerVisitor<MistCore::VisibleNotifier, GridTypeMapContainer  > grid_notifier(notifier);
...
bool Map::ActiveObjectsNearGrid(NGridType const& ngrid) const
{
    ...
    for (MapRefManager::const_iterator iter = m_mapRefManager.begin(); iter != m_mapRefManager.end(); ++iter)
    {
        ...
        CellCoord p = MistCore::ComputeCellCoord(player->GetPositionX(), player->GetPositionY());
...
void Map::AddToActive(Creature* c)
{
    ...
    if (!c->isPet() && c->GetDBTableGUIDLow())
    {
        ...
        {
            GridCoord p2 = MistCore::ComputeGridCoord(c->GetPositionX(), c->GetPositionY());
