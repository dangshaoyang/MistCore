    ...
    public:
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_PACT_OF_THE_DARKFALLEN:
                        {
                            ...
                            if (Is25ManRaid())
                                ++targetCount;
                            MistCore::Containers::RandomResizeList<Player*>(targets, targetCount);
                            if (targets.size() > 1)
                            {
                        ...
                        case EVENT_TWILIGHT_BLOODBOLT:
                        {
                            std::list<Player*> targets;
                            SelectRandomTarget(false, &targets);
                            MistCore::Containers::RandomResizeList<Player*>(targets, uint32(Is25ManRaid() ? 4 : 2));
                            for (std::list<Player*>::iterator itr = targets.begin(); itr != targets.end(); ++itr)
        ...
        private:
            ...
            Player* SelectRandomTarget(bool includeOfftank, std::list<Player*>* targetList = NULL)
            {
                ...
                if (includeOfftank)
                {
                    tempTargets.sort(MistCore::ObjectDistanceOrderPred(me->getVictim()));
